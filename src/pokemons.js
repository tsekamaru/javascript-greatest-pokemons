...
function getAllFirePokemons(Array) {
  const result = Array.filter((item) => item.type.includes("Fire"));
  return result;
}
...
function shortestPokemon(Array) {
  ...
  Array.forEach((pokemon) => {
    if (pokemon.name.length < shortestLength) {
      shortestLength = pokemon.name.length;
      shortestName = pokemon.name;
    }
  });
  return shortestName;
}
...
function candyAverage(Array) {
  let sum = 0;
  Array.forEach((item) => {
    if (item.candy_count !== undefined) sum += item.candy_count;
  });
  let result = Math.round((sum / Array.length) * 100) / 100;
  return result;
}

// Iteration 4: images for the first 10 `Ground`  Pokemons
function getGroundPokeImg(Array) {
  const result = Array.filter((item) => item.type.includes("Ground"))
...
function getHeavyPokemons(Array) {
  let result = structuredClone(Array).filter(
    (pokemon) => parseFloat(pokemon.weight) > parseFloat(Array[24].weight)
  );
  return result;
}
...
function orderAlphabetically(Array) {
  let result = structuredClone(Array).toSorted((a, b) => a.name.localeCompare(b.name));
  return result.slice(0, 20);
}

// Iteration 7: Strong pokemons - return an array of first 15 pokemons, that have just one `weakness`. If there are less that 15, return all of them
function strongPokemons(Array) {
  let result = Array.filter((pokemon) => pokemon.weaknesses.length === 1).slice(0, 15);
  return result;
}
